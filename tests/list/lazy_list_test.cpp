...
class LazyListTest : public ::testing::Test {
 protected:
  void SetUp() override { list_ = new LazyList<int>(); }
...
TEST_F(LazyListTest, EmptyListContainsReturnsFalse) {
  EXPECT_FALSE(list_->contains(1));
}

TEST_F(LazyListTest, AddSuccess) {
  EXPECT_TRUE(list_->add(1));
  EXPECT_TRUE(list_->contains(1));
}

TEST_F(LazyListTest, AddDuplicate) {
  EXPECT_TRUE(list_->add(1));
  EXPECT_FALSE(list_->add(1));
}

TEST_F(LazyListTest, RemoveSuccess) {
  EXPECT_TRUE(list_->add(1));
  EXPECT_TRUE(list_->remove(1));
  EXPECT_FALSE(list_->contains(1));
}

TEST_F(LazyListTest, RemoveNonExistent) {
...
TEST_F(LazyListTest, RemoveMiddleItem) {
  for (size_t i = 1; i <= 5; i++) {
    EXPECT_TRUE(list_->add(static_cast<int>(i)));
  }
...
TEST(OptimisticListCustomTypeTest, BasicOperations) {
  LazyList<TestItem, TestItemHasher> list;
...
// Concurrency tests
TEST_F(LazyListTest, ConcurrentAddDifferentItems) {
...
TEST_F(LazyListTest, ConcurrentAddRemove) {
...
TEST_F(LazyListTest, ConcurrentOperationsMix) {
  constexpr size_t kNumThreads = 4;
  constexpr size_t kOperationsPerThread = 1000;
...
TEST_F(LazyListTest, StressTest) {
...
TEST_F(LazyListTest, HighContentionTest) {
...
TEST_F(LazyListTest, MemoryLeakTest) {
  ...
  for (size_t i = 0; i < 100; i++) {
    if (i % 2 == 0) {
      EXPECT_FALSE(list_->contains(static_cast<int>(i))) << "Value of i " << i;
    } else {
      EXPECT_TRUE(list_->contains(static_cast<int>(i)));
    }
  }
